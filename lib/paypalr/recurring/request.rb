module PayPalr
  module Recurring
    class Request
      ...
      # Do a POST request to PayPalr API.
      ...
      # PayPalr::Recurring::Request::METHODS constant.
      #
      # The params hash can use normalized names. For a list, check the
      # PayPalr::Recurring::Request::ATTRIBUTES constant.
      #
      ...
      def request
        @request ||= Net::HTTP::Post.new(uri.request_uri).tap do |http|
          http["User-Agent"] = "PayPalr::Recurring/#{PayPalr::Recurring::Version::STRING}"
        end
      end
      ...
      #
      def uri # :nodoc:
        @uri ||= URI.parse(PayPalr::Recurring.api_endpoint)
      end
      ...
      def default_params
        {
          :username    => PayPalr::Recurring.username,
          :password    => PayPalr::Recurring.password,
          :signature   => PayPalr::Recurring.signature,
          :version     => PayPalr::Recurring.api_version
        }
      end
