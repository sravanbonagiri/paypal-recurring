module PayPalr
  module Recurring
    class Notification
      extend PayPalr::Recurring::Utils
      ...
      def request
        @request ||= PayPalr::Recurring::Request.new.tap do |request|
          request.uri = URI.parse("#{PayPalr::Recurring.site_endpoint}?cmd=_notify-validate")
        end
      end
      ...
      def valid?
        completed? && verified? && email == PayPalr::Recurring.email && seller_id == PayPalr::Recurring.seller_id
      end
